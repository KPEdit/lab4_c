...
int load(Table *table){
  ...
  char *ts = (char*)malloc(fn + strlen(TABLE_SUME_EXT) + 1);
  strcpy(ts, table->file_name);
  strcat(ts, TABLE_SUME_EXT);
  if ((table->file_sume = fopen(ts, "r+b")) == NULL)
    rc = 1;

  free(td); free(tc); free(ts);
  td = NULL; tc = NULL; ts = NULL;
  ...
  printf("%s\n", "SPACE1 HAS BEEN LOADDED");
  load_space_two(table);
  printf("%s\n", "SPACE2 HAS BEEN LOADDED");
  return 0;
}

int load_space_one(Table *table){
  // loads items from file to table->space1
  Item ritem, *item;
  while (fread(&ritem, sizeof(state)+sizeof(int)*8,1,table->file_sume)){
    item = (Item*)malloc(sizeof(Item));
    *item = ritem;
    item->next = NULL;
    table->space1[item->ind1] = item;
  }
  return 0;
}
...
int close(Table *table){
  ...
  fclose(table->file_sume);
  fclose(table->file_scep);
  return 0;
}

int save_one(Table *table){
  // save data to file file_sume

  char *ts = (char*)malloc(strlen(table->file_name) + strlen(TABLE_SUME_EXT) + 1);
  strcpy(ts, table->file_name);
  strcat(ts, TABLE_SUME_EXT);
  freopen(ts, "w+b", table->file_sume);
  free(ts); ts = NULL;
  for (int i=0;i<TABLE_SIZE; i++){
    if (table->space1[i] != NULL){
      fseek(table->file_sume, 0, SEEK_END);
      fwrite(table->space1[i], sizeof(state)+sizeof(int)*8, 1, table->file_sume);
    }
  }
  return 0;
}
...
int insert(Table *table, Item *item, char *info){
  ...
  printf("%s\n", "INSERT");
  // fclose(table->file_data);
  // char *fd = (char*)malloc(strlen(TABLE_DATA_EXT)+strlen(table->file_name)+1);
  // fopen()
  return 0;
}
...
int delete_one_one(Table *table, int key, int vers){
  ...
  for (int h = hash(key); h < TABLE_SIZE; h+=STEP){
    ...
    if (table->space1[h]->key1 == key && table->space1[h]->ver1 == vers){
      table->space1[h]->busy = DEL;
      rc = 0;
    }
  }
...
int create(Table *table){
  ...
  strcpy(ts, table->file_name);
  strcat(ts, TABLE_SUME_EXT);
  if ((table->file_sume = fopen(ts, "w+b")) == NULL)
    rc = 1;
